 ...
 * @param onFinish - 完成回调
 */
export const spreadDisconnectedComponents = (
  ...
  const compMeta = components.map((comp) => {
    ...
    const radius = Math.sqrt(width * width + height * height) / 2 + 40 * sizeScale;
    const center = {
      x: cx / comp.length,
      y: cy / comp.length,
    };
  ...
  compMeta.forEach((meta) => {
    ...
    meta.comp.forEach((node) => {
      ...
      const relX = (m.x ?? 0) - meta.center.x;
      const relY = (m.y ?? 0) - meta.center.y;
